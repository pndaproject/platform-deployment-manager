...
class ComponentSummary(object):
    ...
    def get_component_summary(self, component, application):
        ...
        status, timestamp = \
        self._application_summary_registrar.get_status_with_timestamp(application)

        job_name = component['component_job_name']
        component_name = component['component_name']
        (aggregate_status, yarnid, tracking_url, information) = ('', '', '', '')
        yarn_data = self._yarn_connection.check_in_yarn(job_name, application)
        if status == 'CREATED':
            if yarn_data != None:
            ...
            if yarn_data != None:
                if timestamp < yarn_data['startedTime']:
                    aggregate_status, yarnid, tracking_url, information = \
                    self.yarn_handler(yarn_data, application)
                else:
