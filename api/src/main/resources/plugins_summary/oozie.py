...
class OozieComponentSummary(ComponentSummary):
    ...
    def get_component_summary(self, component, application):
        ret_data = {}
        data = self._oozie_api_request(component['job_handle'], application)
        ...
        if data['status'] == 'PREP' or data['status'] == 'PREPSUSPENDED':
            ...
            oozie_data = self._oozie_action_handler(data['actions'], application)
    ...
    def _action_yarn_handler(self, yarn_id, app_name):
        yarn_data = self._yarn_connection.yarn_info(yarn_id, app_name)
        (status, information) = ("", "")
    ...
    def _oozie_action_handler(self, actions, app_name):
        """
        ...
        for action in actions:
            if action['externalId'] is not None:
                if 'job_' in action.get('externalId', ''):
                    ...
                    status, information = self._action_yarn_handler(yarn_id, app_name)
                ...
                if 'oozie-oozi-W' in action.get('externalId', ''):
                    ...
                    count += 1
                    oozie_info = self._oozie_api_request(action['externalId'], app_name)
                    oozie_data = self._oozie_action_handler(oozie_info['actions'], app_name)
    ...
    def _oozie_api_request(self, job_id, app_name):
        """
        ...
        try:
            ...
            logging.error('%s while trying to get status of application "%s"', str(error_message), app_name)
        return oozie_info
