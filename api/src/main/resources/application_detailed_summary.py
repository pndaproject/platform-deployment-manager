    ...
    def generate(self):
        ...
        while len(apps_to_be_processed) != 0:
            for app_name in apps_to_be_processed.keys():
                try:
                    app_data = apps_to_be_processed[app_name].task.get(STATUS_INTERVAL) #
                    logging.debug("Application: %s, Status: %s", app_name, app_data[app_name]['aggregate_status'])
